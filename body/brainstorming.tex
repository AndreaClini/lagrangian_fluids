...
We want to relate this to the equation of state $p=p(\rho,s)$, where $\sigma$ is the entropy per particle $\sigma=S/N$.
In particular, we want to highlight to the ratio $w:=p/\rho$ and the adiabatic speed of sound
\begin{equation}
    c_a^2 := \left(\frac{\partial p}{\partial \rho}\right)_\sigma.
\end{equation} 

It is more convvenient to rewrite derivatives in terms of the density $b=\sqrt{B}$, so that 
\begin{equation}
...
and
\begin{align}
    c_s^2 &= \frac{b^2\partial_b^2 F -\frac{\left(bY\partial_{bY}^2F-Y\partial_YF\right)^2}{Y^2\partial_Y^2F}}{b\partial_b F - Y \partial_Y F}.
\end{align}

%----------------------------------------------------------------
\subsubsection{Thermodynamics (temp)} \todotag{Conti a caso, ignore}
%----------------------------------------------------------------


The Euler formula gives
\begin{equation}
    \rho\,V = U = T S - p V + \mu N,
\end{equation}
or equivalently dividing by the volume $V$,
\begin{equation}
    \rho = -p + T \sigma n + \mu n.
\end{equation}
Recall we can identify $\sqrt{B}=n$ as the number density.
...
\begin{align}
    ...
    = \rho + \frac12  \frac{d \rho}{d \log B}_{|\sigma,N}.
\end{align}
{\color{red}
We should identify $Y$ with the temperature $T$, as in \cite{BallesterosComelliPilo_ThermodynamicsPerfectFluidsScalarFieldTheory_2016}.
Then we can write
\begin{align}
...
In turn we should compute
\begin{align}
    c_a^2 &= \left(\frac{\partial p}{\partial \rho}\right)_\sigma
    = \frac{\partial p/\partial B_{|\sigma}}{\partial \rho/\partial B_{|\sigma}} \sim \frac{\partial p/\partial B_{|Y}}{\partial \rho/\partial B_{|Y}},
\end{align}
