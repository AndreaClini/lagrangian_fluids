...
%
{\color{blue}
For the moment we report the relevant formulas
\begin{align}
    \rho &= Y (\partial_Y F) - F, \quad
    \partial_Y\rho= Y \partial_Y^2 F, \quad \partial_b \rho = Y \partial^2_{bY} F - \partial_b F, 
    \\
    p &= F - b (\partial_b F),\quad \partial_Y p =\partial_YF- b \,\partial^2_{bY} F, \quad \partial_b p = - b \partial_b^2 F,
    \\
    w&:=\frac{p}{\rho}=\frac{F - b (\partial_b F)}{Y (\partial_Y F) - F}
    \\
...
Subtracting from the expression for $c_s^2$ in \eqref{eq:nonbarotropic_formulas}, namely
\begin{align}
    c_s^2 &= \frac{\big(Y^2\partial_Y^2F\big)\big(b^2\partial_b^2 F\big) -\big[bY\partial_{bY}^2F-Y\partial_YF\big]^2}{\big(Y^2\partial_Y^2F\big)\big[b\partial_b F - Y \partial_Y F\big]},
\end{align}
{\color{red}
we finally get \todotag{check conto below}
\begin{align}
    c_s^2 - c_a^2
    &= \frac{\big(Y^2\partial_Y^2F\big)\big(b^2\partial_b^2 F\big) -\big[bY\partial_{bY}^2F-Y\partial_YF\big]^2}{\big(Y^2\partial_Y^2F\big)\big[b\partial_b F - Y \partial_Y F\big]}
    +
    \frac{
        \big[Y\partial_Y F- bY\partial_{bY}^2F\big]^2+ b^2 \partial_b^2 F \big[Y \partial_YF - Y^2 \partial_Y^2 F - b \partial_b F\big]
    }
    ...
    \\
    &=
    - \frac{\big[Y\partial_Y F- bY\partial_{bY}^2F\big]^2}{Y^2 \partial_Y^2 F \big[Y \partial_YF- b \partial_b F + b^2 \partial_b^2 F\big]}
    \\
    &=
    - \frac{\left(bY\partial_{bY}^2F - Y\partial_Y F\right)^2}{Y^2 \partial_Y^2 F \left(b \partial_b F - Y \partial_Y F + b^2 \partial_b^2 F\right)}.
\end{align}
This shows that\footnote{assuming convexity in $Y=T$ for god's sake} the non-barotropic contribution to the sound speed is always negative (or zero),
\begin{equation}
    c_s^2 \leq c_a^2,
\end{equation}
with equality holding in the barotropic case $F=F(b)$ where there is no dependence on $Y$ and thus $\partial_Y F=0$ and $\partial_{bY}^2 F=0$.
}
