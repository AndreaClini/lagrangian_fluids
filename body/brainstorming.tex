...
Inside the action we can integrate by parts the term with antisymmetric epsilon tensors, obtaining
{\small
\begin{align}
    ...
    \\
    &= \int d^4x \bigg\{\left[-\bar{B} (\partial_B\bar{F}) + \tfrac12 \bar{Y}(\partial_Y\bar{F})\right] \big(\partial_0 \pi^i \partial_0 \pi^i\big)
    \\
    &\qquad +\bigg[\bar{B} (\partial_B\bar{F}) \,2 + \tfrac12 \bar{Y}(\partial_Y\bar{F}) + \bar{B}^2 (\partial_B^2\bar{F}) \,2\bigg] \,(\partial_i \pi^i)^2
    \\
    &\qquad + \tfrac{1}{2}\bar{Y}^2(\partial_{Y}^2\bar{F}) \big(\partial_0 \pi^0 - \partial_i \pi^i\big)^2
    + \bar{B}\bar{Y} (\partial_{B Y}^2\bar{F}) 2\, \partial_i \pi^i \,\,\big(\partial_0 \pi^0 - \partial_i \pi^i\big)\bigg\}.
\end{align}
}

% {\color{gray}
% The quadratic part of the action can be diagonalized into longitudinal and transverse modes by decomposing
% \begin{equation}
%     \pi^i = \pi_L^i + \pi_T^i, \quad
%     \partial_i \pi_T^i = 0, \quad
%     \epsilon_{ijk} \partial_j \pi_L^k = 0.
% \end{equation}
% The transverse modes have no gradient energy and thus do not propagate, while the longitudinal modes have a speed of sound given by
% \begin{equation}
%     c_s^2 = \dots
% \end{equation}
% }
