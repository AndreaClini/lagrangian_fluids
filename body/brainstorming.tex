...
%============================================================================
\subsection{Setting \& energy-momentum tensor}
%==========================================================================



%-------------------------------------------------------------------
\subsubsection{Setting \& energy-momentum tensor: flat background}
%-------------------------------------------------------------------
...
%-------------------------------------------------------------------
\subsubsection{Setting \& energy-momentum tensor: curved background}
%-------------------------------------------------------------------
%
...
\begin{align}
    T_{\mu\nu}:=-\frac{2}{\sqrt{-g}}\frac{\delta S_{\mathrm{fluid}}}{\delta g^{\mu\nu}}={ \bigl(Y (\partial_Y\!F) - 2 B (\partial_B\!F)\bigr)\,u_\mu u_\nu
           + \bigl(F  - 2 B (\partial_B\!F)\bigr)\,g_{\mu\nu}}\,.
\end{align}
That is, matching to the perfect fluid expression, we get the expressions from \cite{BallesterosComelliPilo_ThermodynamicsPerfectFluidsScalarFieldTheory_2016} \todotag{Does not match flat case... check it back!}
\begin{align}
...
\begin{align}
    ...
    \\
     ...
     \\
     &= -\frac{Y}{2} u_\mu u_\nu\,\,\delta g^{\mu\nu}
     +\underbrace{\frac{1}{3!\,\,\sqrt{B}}\sqrt{-g}\,\epsilon_{\lambda\mu\nu\rho}\,\,\epsilon_{ijk}\,\,\partial_\alpha\phi^0\,\,\partial_\beta \phi^i\,\, \partial_\gamma \phi^j\,\, \partial_\delta \phi^k 
     \,\,\delta\left(g^{\alpha\lambda}g^{\beta\mu}g^{\gamma\nu}g^{\delta\rho}\right)}_{=0 \,\text{by antisymmetry or orthogonality or total derivative?}}
\end{align}
}
Splitting $\partial_\alpha\phi^0= -Y u_\alpha + \left[\partial_\alpha\phi^0 + Y u_\alpha\right]$ might also be useful to compute the variation... In any case let's pretend the last term can be neglected.
...
\begin{align}
    ...
    -\frac{1}{2} g_{\mu\nu} F
    + (\partial_BF)\, B \left(g_{\mu\nu}+u_\mu u_\nu\right) - (\partial_YF)\, \frac{1}{2} Y \left(u_\mu u_\nu\right)
    \\
    &\qquad
    +\partial_YF\,\Big(\text{total derivative or vanishing term}\Big)\bigg\} \delta g^{\mu\nu}.
\end{align}
...
\begin{align}
    ...
    %
    =g_{\mu\nu} F
    -2 (\partial_BF)\, B \left(g_{\mu\nu}+u_\mu u_\nu\right) + (\partial_YF)\, Y \,u_\mu u_\nu
    \\
    &=
    \bigl(Y (\partial_Y\!F) - 2 B (\partial_B\!F)\bigr)\,u_\mu u_\nu
           + \bigl(F - 2 B (\partial_B\!F)\bigr)\,g_{\mu\nu}.
\end{align}
Matching to the perfect fluid expression $T_{\mu\nu}=(\rho+p)u_\mu u_\nu + p g_{\mu\nu}$ we find the expression in Ballesteros et al . \cite{BallesterosComelliPilo_ThermodynamicsPerfectFluidsScalarFieldTheory_2016} \footnote{Note that in \cite{BallesterosComelliPilo_ThermodynamicsPerfectFluidsScalarFieldTheory_2016} they use b=$\sqrt{B}$ and $F(b,Y)$, so that $\partial_b F = 2 \sqrt{B} \partial_B F$.}
\begin{equation}
    \rho = Y (\partial_Y\!F) - F, \qquad
    p = F - 2 B (\partial_B\!F).
\end{equation}
...
%-------------------------------------------------------------------
\subsection{Fluctuations: flat background}
%-------------------------------------------------------------------
...
\begin{align}
    ...
    + O(\pi^3).
\end{align}
Finally the invariant \(Y\) is expanded as 
\begin{align}
...
\begin{align}
    ...
    \\
    &\qquad
    + \bigg[2\,\bar{B}\bar{Y} (\partial_{B Y}^2\bar{F})-\bar{Y}(\partial_{Y}\bar{F})-\bar{Y}^2(\partial_{Y}^2\bar{F})\bigg] \partial_0 \pi^0 \bigg\}
    \\
    &= \partial_0 \left(2\bigg[ \tfrac12 \bar{Y}(\partial_Y\bar{F}) -\bar{B} (\partial_B\bar{F})\bigg] \partial_0 \pi^i\right)
    \\
    &\quad + \partial_i \bigg\{2\bigg[\bar{B} (\partial_B\bar{F}) + 2 \bar{B}^2 (\partial_B^2\bar{F}) + \bar{Y}(\partial_Y\bar{F})
    ...
    \\
    &\qquad\qquad
    -  \frac{\bigg[2\,\bar{B}\bar{Y} (\partial_{B Y}^2\bar{F})-\bar{Y}(\partial_{Y}\bar{F})-\bar{Y}^2(\partial_{Y}^2\bar{F})\bigg]^2}{\bar{Y}^2(\partial_{Y}^2\bar{F})} \, \partial_j \pi^j \bigg\}.
\end{align}
}
%
That is, for the \emph{longitudinal} part $\pi^i_L := \nabla^i \nabla^{-2} \nabla_j \pi^j$ we have
\begin{align}
    \partial_0^2 \pi^i_L
    &- c_s^2 \nabla^2 \pi^i_L = 0,
\end{align}
for the sound speed \todotag{simplify}
\begin{align}
    c_s^2
    &= \frac{\bar{B} (\partial_B\bar{F}) + 2 \bar{B}^2 (\partial_B^2\bar{F}) + \bar{Y}(\partial_Y\bar{F})
    +\tfrac12 \bar{Y}^2(\partial_{Y}^2\bar{F}) -2 \bar{B}\bar{Y} (\partial_{B Y}^2\bar{F})
    -  \frac{\Big(2\,\bar{B}\bar{Y} (\partial_{B Y}^2\bar{F})-\bar{Y}(\partial_{Y}\bar{F})-\bar{Y}^2(\partial_{Y}^2\bar{F})\Big)^2}{2\bar{Y}^2(\partial_{Y}^2\bar{F})}}{ \bar{B} (\partial_B\bar{F}) -\tfrac12 \bar{Y}(\partial_Y\bar{F})}
    \\
    &=
    \frac{\bar{B} (\partial_B\bar{F}) + 2 \bar{B}^2 (\partial_B^2\bar{F}) 
    -  \frac{\Big(2\,\bar{B}\bar{Y} (\partial_{B Y}^2\bar{F})-\bar{Y}(\partial_{Y}\bar{F})\Big)^2}{2\bar{Y}^2(\partial_{Y}^2\bar{F})}}{ \bar{B} (\partial_B\bar{F}) -\tfrac12 \bar{Y}(\partial_Y\bar{F})}
\end{align}
Note that for $F(B,Y)\equiv F(B)$ we recover the barotropic sound speed from \cite{DubovskyGregoireNicolisRattazzi_NullEnergyConditionSuperluminalPropagation_2005}.



It is more convvenient to rewrite derivatives in terms of the density $b:=\sqrt{B}=n$, so that 
\begin{equation}
    B\partial_B = \tfrac12 b \partial_b, \quad B^2\partial_B^2 = \tfrac14 \big(-b \partial_b + b^2 \partial_b^2\big). 
\end{equation}
Then we have
\begin{align}
\label{eq:nonbarotropic_formulas}
    ...
    \\
    c_s^2 &= \frac{b^2\partial_b^2 F -\frac{\left(bY\partial_{bY}^2F-Y\partial_YF\right)^2}{Y^2\partial_Y^2F}}{b\partial_b F - Y \partial_Y F}, \qquad w:=\frac{p}{\rho}=\frac{F - b (\partial_b F)}{Y (\partial_Y F) - F}.
\end{align}



%======================================================
\subsection{Sound speed and equation of state}
%======================================================

Next we want to relate the sound speed $c_s^2$ to the equation of state $p=p(\rho,s)$, where $\sigma$ is the entropy per particle $\sigma:=S/N$.
In particular, we want to highlight to the ratio $w:=p/\rho$ and the \emph{adiabatic} speed of sound
\begin{equation}
    c_a^2 := \left(\frac{\partial p}{\partial \rho}\right)_\sigma.
\end{equation} 

%---------------------------------------------------------------
\subsubsection{Barotropic case}
%---------------------------------------------------------------

In the barotropic case $p= w(\rho) \rho$, there is no dependence on $Y$ and the formulas reduce to
\begin{align}
\begin{aligned}\label{eq:barotropic_formulas}
    \rho &= - F,  \quad \partial_b \rho = - \partial_b F, 
    \\
    p &= F - b (\partial_b F),\quad  \partial_b p = - b \partial_b^2 F,
    \\
    c_s^2 &= \frac{b\partial_b^2 F}{\partial_b F}, \qquad w:=\frac{p}{\rho}=-1 + \frac{b}{F} \,\partial_b F.
\end{aligned}
\end{align}
Moreover in the barotropic case $p= w(\rho) \rho$ we can equivalently compute
\begin{align}\label{eq:barotropic_sound_speed_vs_w}
    c_s^2 &= \frac{\partial p}{\partial \rho} = w + \rho \frac{d w}{d \rho}
    =w + \frac{\frac{dw}{db}}{\frac{1}{\rho}\frac{d\rho}{db}} 
    = w + \frac{\frac{dw}{dt}}{\frac{1}{\rho}\frac{d\rho}{dt}} \frac{\graycancel{\frac{dt}{db}}}{\graycancel{{\frac{dt}{db}}}}.
\end{align}
From the expression \eqref{eq:barotropic_formulas} for $w=w(b)$ we have
\begin{align}
    \frac{dw}{db} &= \frac{d}{db}\left(-1 + \frac{b}{F} \,\partial_b F\right)
    = \frac{\partial_b F + b \,\,\partial_b^2 F}{F} - \frac{b \big(\partial_b F\big)^2}{F^2}.
\end{align}
Using these two and the expressions \eqref{eq:barotropic_formulas} for $\rho=\rho(b)$ and $\partial_b \rho$ we can verify that indeed the two expressions in \eqref{eq:barotropic_formulas} and \eqref{eq:barotropic_sound_speed_vs_w} for $c_s^2$ coincide.
Indeed
\begin{align}
    c_s^2 &= w + \frac{\frac{dw}{db}}{\frac{1}{\rho}\frac{d\rho}{db}} 
    = \redcancel{-1} + \bluecancel{\frac{b}{F} \,\partial_b F} + \frac{\frac{\redcancel{\partial_b F}+ b \,\,\partial_b^2 F}{F} - \bluecancel{\frac{b \big(\partial_b F\big)^2}{F^2}}}{\frac{1}{-F}(-\partial_b F)}
    = \frac{b\,\partial_b^2 F}{\partial_b F}.
\end{align}



%----------------------------------------------------------------
\subsubsection{Non-barotropic case \& thermodynamics refresher (temp)} \todotag{Conti a caso, ignore}
%----------------------------------------------------------------
...
Subtracting $c_a^2$ from the expression for $c_s^2$ in \eqref{eq:nonbarotropic_formulas},
{\color{red}
we finally get
\begin{align}
    c_s^2 - c_a^2
    &= \frac{
        \big[(Y\partial_Yp)^2 +  (Y\partial_Y\rho)(b\partial_b p)\big] \,
        \big[(b\partial_b \rho)^2 - (Y\partial_Y\rho)\big((\rho+p) - (b\partial_b p)\big)\big]
    }{
        (Y\partial_Y\rho)\,\,(\rho+p) \,
        \big[(b\partial_b \rho)^2 - (Y\partial_Y\rho)\big((\rho+p) - (b\partial_b p)\big)\big]
    }
    ...
    &\quad +
    \frac{
        \big[(Y\partial_Yp)^2 -(b\partial_b p)\big[(\rho+p) - (Y\partial_Y\rho)\big]\big] \,
        \big[\big(Y \partial_YF - b \partial_b F\big) + b^2 \partial_b^2 F\big]
    }{
        \big[ bY\partial_{bY}^2F - b\partial_b F\big]^2 - Y^2 \partial_Y^2 F \big[ \big(Y \partial_YF - b \partial_b F\big) + b^2 \partial_b^2 F \big]
    }
    \\
    &=
    \frac{
        (Y\partial_Yp)^2 \, (b\partial_b \rho)^2
        - (Y\partial_Y\rho)^2 \, (b\partial_b p)^2
        + (Y\partial_Y\rho)(b\partial_b p)^2 (\rho+p)
        - (Y\partial_Yp)^2 (Y\partial_Y\rho)(\rho+p)
    }{
        (Y\partial_Y\rho)\,\,(\rho+p) \,
        \big[(b\partial_b \rho)^2 - (Y\partial_Y\rho)\big((\rho+p) - (b\partial_b p)\big)\big]
    }
...
We would like to express the offset in terms of $w$ and its derivatives and possibly show it stays positive.
...
{\color{violet}
\noindent
\textbf{Strategy 1: expressing $c_s^2-c_a^2$ via thermodynamic potentials.}
\smallskip

\noindent
To show positivity of $c_s^2 - c_a^2$ it might be useful to express everything in terms of thermodynamic potentials like $F=F(T,n)$ or $G=G(T,p)$.
...
\begin{align}
    ...
    = T \frac{\partial (U/V)}{\partial S}_{|V} \frac{-\partial \,(\partial_T F)_{|V}}{\partial T}_{|V}
    = -\frac{T^2}{V} \, \partial_T^2 F_{|V},
    \\
    ...
    =  \rho - \frac{\partial (F+TS)}{\partial V}_{|T}
    =  \rho + p +T \frac{\partial^2 F}{\partial T \partial V},
    \\
    T \frac{\partial P}{\partial T}_{|n} &= T \frac{\partial P}{\partial T}_{|V} = T \frac{\partial }{\partial T}_{|V}\left(- \frac{\partial F}{\partial V}_{|T}\right) = - T \frac{\partial^2 F}{\partial T \partial V},
    \\
    n \frac{\partial P}{\partial n}_{|T} &= -V\frac{\partial P}{\partial V}_{|T} = V \frac{\partial^2 F}{\partial^2 V}_{|T}.
\end{align}
Using convexity of $F$  and rewriting $c_s^2 - c_a^2$ in terms of its derivatives we might be able to show positivity...

\begin{align}
    c_s^2 &= \frac{(T\partial_Tp)^2 +  (T\partial_T\rho)(n\partial_n p)}
    {(T\partial_T\rho)\,\,(\rho+p)}
    \\
    &= V \cdot \frac{(\partial_V^2F_{|T})(\partial_T^2F_{|V})- \big(\partial_{VT}^2F\big)^2}{(\rho+P)(\partial_T^2F_{|V})}
    = V\cdot \frac{\det(\partial^2_{V,T} F)}{(\rho+P)(\partial_T^2F_{|V})}
    \\
    ...
    \\
    &= -\frac{
        \big( T \partial_{VT}^2F\big)^2 - \big(V \partial_V^2F\big) \big[(\rho+p) + \tfrac{T^2}{V} \partial_T^2F\big]
    }
    {
        \big[(\rho+P)+ T \partial_{VT}^2F\big]^2 +\big(\tfrac{T^2}{V}\partial_T^2F\big) \big[(\rho+p)  - V \partial_V^2F\big]
    }
    = check last expr
\end{align}
...
{\color{forestgreen}
\noindent
\textbf{Equivalent computations of $c_a^2$, to make contact with barotropic case.}
\smallskip
...
\end{align}
This method yields the same expression for $c_a^2$ as above. \todotag{Plug in the values and check it}
We should however express $c_s^2$ in a similar fashion to compare them more easily \& highlight the nonbarotropic effect. \todotag{This might be the key, do it }
}
\bigskip
...
{\color{forestgreen}
\noindent
\textbf{Strategy 2: expressing $c_s^2-c_a^2$ via the EoS.}
\smallskip
...
{\color{orange}
\noindent
\textbf{Strategy 3 (the only possibly working): use $w$ and $\rho$ as your variables.}
